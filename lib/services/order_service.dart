...
class OrderService {
  ...
  Future<void> addToCart(
    ...
    if (orderDoc.exists) {
      ...
      await orderRef.set({
        ...
        "totalPrice": price * quantity,
        "unpaidTotal": price * quantity,
        "createdAt": FieldValue.serverTimestamp(),  // **Added timestamp here**
        "items": [
          {
            "name": name,
            "price": price,
  ...
  Future<List<Map<String, dynamic>>> getGroupedOrders(String userEmail) async {
    ...
    await cartCollection.doc(userEmail).collection("orders").orderBy("createdAt", descending: true).get(); // **Sorting by time**
